/*
  ...
  VERSION:
  	 April 2018: modified by Alvaro Cassinelli
*/
...
class DueTimer
{
  ...
  public:
    ...
    DueTimer& start(uint32_t microseconds);
    DueTimer& start(void);
    DueTimer& stop(void);

    bool getRunningState() {return(runningState);}
    bool getPauseState() {return(pauseFlag);}

    DueTimer& pause(void); //does not affet runningState
    DueTimer& resume(void);//no effect if it was not running

    DueTimer& setFrequency(double frequency);
    DueTimer& setPeriod(uint32_t microseconds);

    //bool getState(void) const; //return runningState;
