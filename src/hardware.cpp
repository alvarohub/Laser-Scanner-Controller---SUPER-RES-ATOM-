...
namespace Hardware {
	...
	namespace Gpio {
		...
		void init() { // set default modes, output values, etc of pins, other that the laser and scanner:
			...
			void setPWMFreq(FREQ_PWM); // frequency in HZ.
		...
		// they are set to 1kHz]. The code below affect ALL PWM pins.
		...
		void setPWMDuty(uint8_t pinPWM, uint16_t _duty) { // _duty is from 0 to 2047 (11 bit res)
			if (_duty<2048) PWM->PWM_CH_NUM[i].PWM_CDTYUPD = _duty;
		}
	}
