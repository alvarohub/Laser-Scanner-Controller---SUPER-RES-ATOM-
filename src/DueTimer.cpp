...
DueTimer& DueTimer::detachInterrupt(void) {
  stop(); // Stop the currently runningState timer
...
DueTimer& DueTimer::start() {
  runningState = true;

  // if(microseconds > 0)
  // 	setPeriod(microseconds);

  if (_frequency[timer] <= 0) setFrequency(1);
...
DueTimer& DueTimer::pause(void) {
  // Disable interrupt, but don't change the current runningState state.
  // However, signal the pauseFlag (for external check):
  pauseFlag = true;
  NVIC_DisableIRQ(Timers[timer].irq);
  TC_Stop(Timers[timer].tc, Timers[timer].channel);
  return *this;
...
DueTimer& DueTimer::resume(void) {
pauseFlag = false;
  if (runningState) { // re-enable interrupt if runningState was true
    NVIC_DisableIRQ(Timers[timer].irq);
    TC_Stop(Timers[timer].tc, Timers[timer].channel);
  }
  return *this;
